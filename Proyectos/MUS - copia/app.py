...
def perfilusuario():
    ...
    if leidos != 1:
         flash('No se encontró el perfil')
    else:
...
def storage():
    ...
    if leidos > 0 and (datosusuario [0] [6] == True):
    ...
    if leidos > 0 and (datosusuario [0] [6] == False):
        print("DEBUG. ALTA USUARIO: codigo verificacion BD: ", datosusuario [0] [7]) 
        verificacion = request.form['verificacion'] 
        if datosusuario [0] [7] != verificacion:            
            ...
            return render_template('entrarajugar.html',usuario=alias,nombre=nombrep)
    else:  
        CUsuarios.modificarActivacion(alias)
        session['usuario'] = alias
        return render_template('entrarajugar.html',usuario=alias,nombre=nombrep)
...
def iniciar_partida(data):
    ...
    try:
        emit('partida_iniciada', { "mesa_id": table_id, "mesa": mesa}, room=table_id)
...
def iniciar_partida(data):
    ...
    emit('reiniciar_nueva_partida', { "mesa_id": table_id}, room=table_id)
...
def pasar_a_siguiente_lance(mesa):
    ...
    if indice_actual + 1 < len(lances):
        ...
        if mesa["lance_actual"] == "Pares":
            ...
            emit("pares_confirmados", {'resultados_pares': mesa["pares_confirmados"], 'lance': mesa["lance_actual"], 'total_pares': total_pares, 'turno_actual': turno_actual, 'contrarias': contrarias, 'puntos': mesa['puntos']}, broadcast=True)
            socketio.sleep(1)
            """
